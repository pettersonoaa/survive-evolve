...
## Run: godot --path . --headless -s res://scripts/tools/generate_partner_sheet.gd

const _Factory = preload("res://scripts/art/wolf_sprite_factory.gd")
const OUTPUT := "res://assets/sprites/partner/partner_sheet.png"
const BODY_COLOR := Color(0.52, 0.52, 0.55)
...
func _generate() -> void:
	...
	for i in WALK_FRAMES:
		...
		var tex := _Factory.create(BODY_COLOR, BODY_SIZE + Vector2(absf(sway) * 0.15, 0), int(sway))
		_blit(sheet, tex, (IDLE_FRAMES + i) * CELL_W, 0)

	var err := sheet.save_png(ProjectSettings.globalize_path(OUTPUT))
	if err != OK:
		push_error("Failed to write partner_sheet.png (%s)" % err)
	else:
		print("Wrote ", OUTPUT)
